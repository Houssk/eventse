angular.module('events.gallery', [])

.directive('ngGallery', ['$document', '$timeout', '$q', '$templateCache', '$http', function($document, $timeout, $q, $templateCache, $http) {
  'use strict';
  ...
  function setScopeValues(scope, attrs) {
    ...
    scope.deleteOption = scope.deleteOption || false;
    scope.confirmDeletePicture = false;
  }
  ...
  '    <img ng-src="{{ i.thumb }}" class="img-rounded {{ thumbClass }}" ng-click="openGallery($index)" alt="Image {{ $index + 1 }}" />' +
  '  </div>' +
  '</div>' +
  ...
  '  <a class="trash-popup" ng-show="deleteOption && !confirmDeletePicture" ng-click="confirmDeletePicture = true"><i class="fa fa-trash-o"></i></a>' +
  '  <span class="confirmDelete-popup" ng-show="confirmDeletePicture">Supprimer ? <a ng-click="removePicture()"><i class="fa fa-check"></i><a/> <a ng-click="confirmDeletePicture = false"><i class="fa fa-remove"></i></a></span>' +
  ...
  return {
    ...
    link: function (scope, element, attrs) {
      ...
      var loadImage = function (i) {
        ...
        var image = new Image();
        scope.confirmDeletePicture = false; //close the confirm delete option
      ...
      scope.removePicture = function () {
        $http.delete('/api/picture/' + scope.eventId + '/' + scope.images[scope.index].id)
          .success(function(data) {
            ...
            for (var i=0; i < scope.images.length; i++) {
              if (scope.images[i].id == scope.images[scope.index].id) {
                index = i;
                break;
              }
            }
            ...
            if (scope.index != 0) {
              scope.changeImage(scope.index-1);
            }
            else if (scope.index < scope.images.length) {
              scope.changeImage(0);
            }
            else {
              scope.closeGallery(); 
